...
@section('title', 'Update Data Rekam Medis')

@section('header', 'Update Data Rekam Medis')
            ...
            <form action="{{ route('rekamMedis.update', $rm->id) }}" method="POST">
                ...
                <div class="px-4 py-5 sm:p-6">
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama" value="{{ $rm->pasien->nama }}" disabled
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="poli_id" value="{{ $rm->dokter->poli->nama_poli }}" disabled
                            class="w-full px-3 py-2 text-sm transition duration-300 border rounded-md shadow-sm pointer-events-none bg-slate-200 placeholder:text-slate-400 text-slate-700 border-slate-200 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 focus:shadow" />
                    </div>
                    <div class="mb-4">
                        ...
                        <textarea type="text" name="keluhan" placeholder="Keluhan"
                            class="w-full px-3 py-2 text-sm transition duration-300 bg-transparent border rounded-md shadow-sm placeholder:text-slate-400 text-slate-700 border-slate-200 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 focus:shadow">{{ $rm->keluhan }}</textarea>
                    </div>
                    <div class="mb-4">
                        <label class="block mb-1 text-sm font-medium text-gray-700">Diagnosa</label>
                        <input type="text" name="diagnosa" placeholder="Diagnosa" value="{{ $rm->diagnosa }}"
                    ...
                    <div class="mb-4">
                        ...
                        <select name="tindakan_id"
                            ...
                            @foreach ($tindakan as $p)
                                <option value="{{ $p->id }}" {{ $p->id == $rm->tindakan_id ? 'selected' : '' }}>
                                    {{ $p->nama_tindakan }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea type="text" name="resep" placeholder="Resep Obat"
                            class="w-full px-3 py-2 text-sm transition duration-300 bg-transparent border rounded-md shadow-sm placeholder:text-slate-400 text-slate-700 border-slate-200 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 focus:shadow"> {{ $rm->resep }}</textarea>
                    </div>
                    <div class="mb-4">
                        <label class="block mb-1 text-sm font-medium text-gray-700">Obat</label>
                        <div id="obat-container">
                            @foreach ($rm->obatrm as $item)
                        ...
                        <div class="mb-4">
                            ...
                            <textarea type="text" name="ket" placeholder="Keterangan"
                                class="w-full px-3 py-2 text-sm transition duration-300 bg-transparent border rounded-md shadow-sm placeholder:text-slate-400 text-slate-700 border-slate-200 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 focus:shadow">{{ $rm->ket }}</textarea>
                        </div>
                    </div>
                    <button type="submit"
